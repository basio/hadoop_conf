...
grep finishSuperStep -r "$1"|grep BASIO|awk  '{ gsub(":"," ",$1); print $1,$2,$8}'|sed -e 's/\(.*\)job_\(.*\)_\(.*\)\/\(.*\)m_\(.*\)_0\/syslog/job \2_\3 attempt \5/'|awk '{ gsub(",",".",$6); print "insert into ss values(\"",$2,"\",",$4,",\"",$5,$6,"\",",$7,");"}'
...
awk '{if ($1=="a") print $2,$3}' powergraph.b |awk '{ print "insert into graph values (",$1,",",$2,")");}'
